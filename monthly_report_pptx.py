...
_s1_prompt = f"""{_base}
{_s1_data}
請輸出3條市場觀察，格式嚴格如下（共6行，絕對不可有空白行，嚴禁其他內容）：
01)標題（5字以內）
說明（25字以內，含具體數字）
02)標題（5字以內）
說明（25字以內，含具體數字）
...
請針對以上排行榜進行深度分析，寫出2句重點觀察（共2句，每句約35-45字）。
要求：
1. 必須引用具體的「代表性建案名稱」與「行政區」來佐證說明。
2. 第1句請點出高價區（如西屯、南屯）的豪宅板塊動能或創價能力。
3. 第2句請點出成交量熱區的剛需買盤特徵或聚落效應。
嚴禁條列式編號，直接輸出兩句純文字。"""
...
行政區成交排行（前10）：
{_dist_data}
請輸出3組行政區市場深度觀察，【嚴格遵守格式】必須剛好6行，絕對不可有空白行：
標題1（6字以內）
說明1（30-40字，結合數據點出熱門區的買盤動能或集中原因）
標題2（6字以內）
說明2（30-40字，結合數據分析高單價區的價格支撐力或特徵）
標題3（6字以內）
說明3（30-40字，結合數據點出低價區現況或與市中心的價格落差）"""
...
請根據歷史趨勢判斷市場目前處於盤整、冷卻或成長期。
【極度重要】請嚴格遵守以下格式，必須剛好4行，絕對不可有空白行，嚴禁使用任何 Markdown 符號(如 ** 或 #)：
市場判讀標題（6字以內）
市場判讀說明（35-50字，剖析均單與量能趨勢背後的市場心理與動能）
本月總結標題（6字以內）
本月總結說明（35-50字，綜合評估市場現況並給出下階段展望）"""
